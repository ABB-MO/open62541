...
multiCastType(UA_ADDRINFO *info) {
    const UA_Byte *address;
    if(info->ai_family == AF_INET) {
        address = (UA_Byte *)&((UA_SOCKADDR_IN *)info->ai_addr)->sin_addr;
        if(isMulticastAddress(address, IPV4_PREFIX_MASK, IPV4_MULTICAST_PREFIX))
            return MULTICASTTYPE_IPV4;
#if UA_IPV6
    } else if(info->ai_family == AF_INET6) {
        address = (UA_Byte *)&((UA_SOCKADDR_IN6 *)info->ai_addr)->sin6_addr;
...
setMulticastInterface(const char *netif, UA_ADDRINFO *info,
    ...
    for(ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        char host[UA_NI_MAXHOST];
        ret = UA_getnameinfo(ifa->ifa_addr,
                          (info->ai_family == AF_INET) ?
                          sizeof(UA_SOCKADDR_IN) : sizeof(UA_SOCKADDR_IN6),
                          host, UA_NI_MAXHOST, NULL, 0, NI_NUMERICHOST);
        if(ret != 0) {
            UA_LOG_SOCKET_ERRNO_WRAP(
               UA_LOG_ERROR(logger, UA_LOGCATEGORY_SERVER,
...
static UA_StatusCode
setupListenMultiCast(UA_FD fd, UA_ADDRINFO *info, const UA_KeyValueMap *params,
...
static UA_StatusCode
setupSendMultiCast(UA_FD fd, UA_ADDRINFO *info, const UA_KeyValueMap *params,
                   MultiCastType multiCastType, const UA_Logger *logger) {
    MulticastRequest req;
    UA_StatusCode res = setupMulticastRequest(fd, &req, params, info, logger);
    if(res != UA_STATUSCODE_GOOD)
...
UDP_connectionSocketCallback(UA_POSIXConnectionManager *pcm, UDP_FD *conn,
    ...
    switch(source.ss_family) {
        case AF_INET:
            UA_inet_ntop(AF_INET, &((UA_SOCKADDR_IN *)&source)->sin_addr,
                         sourceAddr, 64);
            sourcePort = UA_htons(((UA_SOCKADDR_IN *)&source)->sin_port);
            break;
        case AF_INET6:
            UA_inet_ntop(AF_INET6, &(((UA_SOCKADDR_IN6 *)&source)->sin6_addr),
                         sourceAddr, 64);
            sourcePort = UA_htons(((UA_SOCKADDR_IN6 *)&source)->sin6_port);
            break;
        default:
            sourceAddr[0] = 0;
            sourcePort = 0;
...
UDP_registerListenSocket(UA_POSIXConnectionManager *pcm, UA_UInt16 port,
                         UA_ADDRINFO *info, const UA_KeyValueMap *params,
    ...
    if(mc != MULTICASTTYPE_NONE) {
        if(info->ai_family == AF_INET) {
            UA_SOCKADDR_IN *orig = (UA_SOCKADDR_IN *)info->ai_addr;
            UA_SOCKADDR_IN sin;
            memset(&sin, 0, sizeof(sin));
            sin.sin_family = AF_INET;
            sin.sin_addr.s_addr = UA_htonl(INADDR_ANY);
            sin.sin_port = orig->sin_port;
            ret = UA_bind(listenSocket, (UA_SOCKADDR*)&sin, sizeof(sin));
        } else if(info->ai_family == AF_INET6) {
            UA_SOCKADDR_IN6 *orig = (UA_SOCKADDR_IN6 *)info->ai_addr;
            UA_SOCKADDR_IN6 sin6;
            memset(&sin6, 0, sizeof(sin6));
            sin6.sin6_family = AF_INET6;
            sin6.sin6_addr = UA_in6addr_any;
            sin6.sin6_port = orig->sin6_port;
            ret = UA_bind(listenSocket, (UA_SOCKADDR*)&sin6, sizeof(sin6));
        }
    } else {
        ret = UA_bind(listenSocket, info->ai_addr, (UA_SOCKLEN)info->ai_addrlen);
    }
#else
    ...
    do {
        ...
        do {
            ...
            n = UA_sendto((UA_FD)connectionId, (const char*)buf->data + nWritten,
                          bytes_to_send, flags, (UA_SOCKADDR*)&conn->sendAddr,
                          conn->sendAddrLength);
            if(n < 0) {
...
registerSocketAndDestinationForSend(const UA_KeyValueMap *params,
                                    const char *hostname, UA_ADDRINFO *info,
