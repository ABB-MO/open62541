...
#include <oul.pf.option.h>

#if OUL_PF_O_NET_MEMORY_UNCACHED_POOL

#include <oul.memory.h>

static struct OUL_Memory                    s_tUncachedPool = { .bInitialised = false };
static OUL_PF_O_NET_MEMORY_UNCACHED uint8_t s_au8UncachedPool[OUL_PF_O_NET_MEMORY_UNCACHED_SIZE];

static void UA_EventLoop_addUncachedMemory(void) {
    OUL_MemoryAdd(&s_tUncachedPool, s_au8UncachedPool, sizeof(s_au8UncachedPool));
}

static UA_StatusCode UA_EventLoop_allocUncachedBuffer(UA_ByteString* ptString, size_t uxBytes) {
    OUL_MemoryInitialise(&s_tUncachedPool, UA_EventLoop_addUncachedMemory);
    ptString->data   = OUL_MemoryAllocateAligned(&s_tUncachedPool, uxBytes, __FUNCTION__, 32, 8);
    ptString->length = NULL == ptString->data ? 0 : uxBytes;
    return NULL == ptString->data ? UA_STATUSCODE_BADOUTOFMEMORY : UA_STATUSCODE_GOOD;
}
#endif // OUL_PF_O_NET_MEMORY_UNCACHED_POOL
...
#if OUL_PF_O_NET_MEMORY_UNCACHED_POOL
        return UA_EventLoop_allocUncachedBuffer(&pcm->rxBuffer, rxBufSize);
#else
...
UA_EventLoopPOSIX_setNonBlocking(UA_FD sockfd) {
    ...
    int nError = OUL_NET_SocketBlockingModeSet(sockfd, false);
    return 0 == nError ? UA_STATUSCODE_GOOD : UA_STATUSCODE_BADINTERNALERROR;
